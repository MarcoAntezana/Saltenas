...
public class MoverPlayer2 : MonoBehaviour
{
    ...
    void Update()
    {
        Vector2 entradaJugador = new Vector2(Input.GetAxis("Horizontal2"), Input.GetAxis("Vertical2"));
        if (entradaJugador.magnitude > 0.1f) // Usa magnitude para evitar falsos positivos con valores mínimos.
        {
            animadorSapo.SetBool("corre", true); // Activa la animación de correr
        }
        else
        {
            animadorSapo.SetBool("corre", false); // Detiene la animación de correr
        }
        ...
        if (Input.GetButtonDown("Fire2"))
        {
            ...
            animadorSapo.SetTrigger("empuja");
            ApplyPush();
        }
        ...
        if (Input.GetButtonDown("Jump2") && pisando)
        {
